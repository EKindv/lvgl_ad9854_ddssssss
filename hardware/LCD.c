...
//---------------- PA?? -------------------
// ??PA15 (SPI_CS) ????
GPIOA->CRH &= ~(0xF << 28);       // ??PA15???(CRH[31:28])
GPIOA->CRH |= (0x3 << 28);        // 50MHz??????(???11)
GPIOA->ODR |= 1<<15;              // ??????

//---------------- PB?? -------------------
// ??PB3 (SPI_SCK) ????
GPIOB->CRL &= ~(0xF << 12);       // ??PB3???(CRL[15:12])
GPIOB->CRL |= (0x3 << 12);        // 50MHz????

// ??PB4 (SPI_SDO_RD) ????
GPIOB->CRL &= ~(0xF << 16);       // ??PB4???(CRL[19:16])
GPIOB->CRL |= (0x4 << 16);        // ??????(CNF=01)

// ??PB5 (SPI_SDI) ????
GPIOB->CRL &= ~(0xF << 20);       // ??PB5???(CRL[23:20])
GPIOB->CRL |= (0x3 << 20);        // 50MHz????

//---------------- PD?? -------------------
// ??PD6 (SPI_DC)?PD7 (SPI_RST)?PD15 (LCD_LED) ????
// PD6??(CRL[27:24])
GPIOD->CRL &= ~(0xF << 24);
GPIOD->CRL |= (0x3 << 24);        // 50MHz????

// PD7??(CRL[31:28])
GPIOD->CRL &= ~(0xF << 28);
GPIOD->CRL |= (0x3 << 28);        // 50MHz????

// PD15??(CRH[31:28])
GPIOD->CRH &= ~(0xF << 28);
GPIOD->CRH |= (0x3 << 28);        // 50MHz????

// ??????
GPIOD->ODR |= (1<<6)|(1<<7)|(1<<15); // DC?RST?LED?????
