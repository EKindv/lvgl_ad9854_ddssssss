...
void TP_Save_Adjdata(void)
{
	...
	temp=tp_dev.yfac*100000000;//保存y校正因素    
			...
			tempbuf[7]=temp>>24;
	//保存x偏移量
			tempbuf[8]=tp_dev.xoff;
			tempbuf[9]=tp_dev.xoff>>8;
	//保存y偏移量
			tempbuf[10]=tp_dev.yoff;
			tempbuf[11]=tp_dev.yoff>>8;
	//保存触屏类型
			tempbuf[12]=tp_dev.touchtype;
	temp=0X0B;//标记校准过了
			tempbuf[13]=temp;
			W25Q16_Write(tempbuf,SAVE_ADDR_BASE,14);		
...
{					  
	s32 tempfac;
	u8 tempbufa[14];
 if(W25QX_Flash_ReadID()==W25Q32)
	{
	W25Q16_Read(tempbufa,SAVE_ADDR_BASE,14);
	tempfac=tempbufa[13];	
	if(tempfac==0X0B)	
	{    												 
		tempfac=0;
	    ...
	    //得到x偏移量
		tp_dev.xoff=0;
		tp_dev.xoff|=tempbufa[9];
		tp_dev.xoff<<=8;
		tp_dev.xoff|=tempbufa[8];
	
 	    //得到y偏移量
		tp_dev.yoff=0;
		tp_dev.yoff|=tempbufa[11];
		tp_dev.yoff<<=8;
		tp_dev.yoff|=tempbufa[10];		
		
		
		tp_dev.touchtype=tempbufa[12];	
...
u8 TP_Init(void)
{
	...
	//TCS  = PA7	
	u8 temp=30;
	if((tp_dev.touchtype&&0X80)==1)		 	//电容触摸屏		
	{
		FT6336_Init();
	...
	}else
	{
		...
		if(TP_Get_Adjdata())
		{
			while(temp--)
			{
					...
					{
						POINT_COLOR=RED;	
						Show_Str(73,102,"电阻触摸测试",16,1);
						Show_Str(48,132,"如需要重新校准触摸!",16,1);
						Show_Str(35,152,"请点击屏幕!否则请等待!",16,1);
